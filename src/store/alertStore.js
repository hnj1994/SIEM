...
export const useAlertStore = create()((set, get) => ({
    ...
    getFilteredAlerts: () => {
        ...
        return alerts.filter((alert) => {
            ...
            if (now.getTime() - alertTime > timeRangeMs)
                return false;
            if (filters.severity !== 'all' && getRuleLevel(alert.rule.level) !== filters.severity)
                return false;
            if (filters.agent && !alert.agent.name.includes(filters.agent))
                return false;
            if (filters.search) {
                const search = filters.search.toLowerCase();
                if (!alert.rule.description.toLowerCase().includes(search) &&

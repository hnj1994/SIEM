...
function StatusIcon({ status }: { status: string }) {
  if (status === 'pass') return <CheckCircle size={16} style={{ color: '#10b981' }} />;
    ...
    <Layout title="Compliance & Frameworks">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        {FRAMEWORKS.map((framework) => {
          ...
          return (
            <div key={framework.name} className="card">
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <h3 style={{ fontSize: 16, fontWeight: 700 }}>{framework.name}</h3>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>{pass}/{total} controls</span>
                  <div style={{
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 8 }}>
                {framework.controls.map((control) => (
                  <div key={control.id} style={{
                    ...
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 2 }}>{control.id}. {control.name}</div>
                      <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{control.desc}</div>
                    </div>
                  </div>
                ))}
              </div>

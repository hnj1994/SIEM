...
export function generateMockAlerts(count: number = 200): WazuhAlert[] {
...
export function generateMockAgents(): Agent[] {
  ...
  const statuses: Agent['status'][] = ['active', 'active', 'active', 'active', 'disconnected', 'pending'];
  
  return agents.map((name, i) => {
    const platform = platforms[Math.floor(Math.random() * platforms.length)] as keyof typeof osNames;
